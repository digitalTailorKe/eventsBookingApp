...
const OnsiteRegistrationForm = () => {
    ...
    if (validateForm()) {
      ...
      axiosClient
        ...
        .then(({ data }) => {
          ...
          // console.log(data);
          navigateToGatePassPage(data.data.full_name, data.data.id);
        })
        .catch((error) => {
          setLoading(false);
  ...
  const navigateToGatePassPage = (name, id) => {
    window.location.href = `https://indoeastafricaexpo.org/gatepass?name=${name}&id=${encryptIds(
      id
    )}`;
  };
